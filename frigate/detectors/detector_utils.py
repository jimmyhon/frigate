...
def tflite_init(self, interpreter, detector_config: BaseDetectorConfig):
    ...
    if self.model_type == ModelTypeEnum.yologeneric:
        ...
        _tflite_generate_anchors_and_strides(self)  # decode bounding box DFL
...
def _tflite_generate_anchors_and_strides(self):
...
def tflite_determine_indexes_for_non_yolo_models(self):
...
def tflite_pre_process(self, tensor_input):
...
def tflite_detect_raw(self, tensor_input):
    tensor_input = tflite_pre_process(self, tensor_input)
    ...
    if self.model_type == ModelTypeEnum.yologeneric:
        ...
        tflite_determine_indexes_for_non_yolo_models(self)
